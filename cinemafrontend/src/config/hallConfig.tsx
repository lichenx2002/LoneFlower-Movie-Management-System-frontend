...
import { getAislePositions } from './aisleAlgorithm';
...
interface HallTypeConfig {
  aislePositions: number[] | ((colCount: number) => number[]);  // 过道位置（列号），可为数组或函数
  seatTypes: {  // 座位类型配置
...
export const hallConfig: Record<string, HallTypeConfig> = {
  // 大型影厅配置
  LARGE: {
    aislePositions: (colCount: number) => getAislePositions(colCount, 2, false),
    seatTypes: {
      STANDARD: {  // 标准座位（大写）
        color: '#8c8c8c',
