...
const OrderManagement: React.FC = () => {
  ...
  // 审核退票申请
  const handleReviewRefund = async (orderId: number, approved: boolean) => {
    try {
      await orderAPI.reviewRefund(orderId, approved);
      message.success(approved ? '已同意退票申请' : '已拒绝退票申请');
      fetchOrders(); // 刷新列表
    } catch (error) {
      message.error('审核失败');
      console.error('审核退票申请错误:', error);
    }
  };
          ...
          return (
            <div style={{ display: 'flex', gap: 8 }}>
              <Button
                type="primary"
                size="small"
                onClick={() => handleReviewRefund(record.orderId, true)}
              >
                同意退票
              </Button>
              <Button
                danger
                size="small"
                onClick={() => handleReviewRefund(record.orderId, false)}
              >
                拒绝退票
              </Button>
            </div>
