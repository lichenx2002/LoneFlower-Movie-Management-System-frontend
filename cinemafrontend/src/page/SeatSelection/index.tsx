...
const SeatSelection: React.FC = () => {
    ...
    const handleScheduleChange = (newScheduleId: number) => {
        navigate(`/seat/movies/${scheduleDetail?.movie.movieId}/schedule/${newScheduleId}`);
    };
    ...
    const maxColCount = React.useMemo(() => {
        ...
        const allRows = scheduleDetail.seats.reduce((acc, seat) => {
            if (!acc[seat.rowLabel]) acc[seat.rowLabel] = [];
            acc[seat.rowLabel].push(seat.colNum);
            return acc;
        }, {} as Record<string, number[]>);
        let max = 0;
        Object.values(allRows).forEach(cols => {
    ...
    const renderSeatsInRow = (rowLabel: string) => {
        ...
        // 以 colNum 为 key，方便查找
        const seatMap = new Map(rowSeats.map(seat => [seat.colNum, seat]));

        // 动态决定过道
        const colCount = maxColCount; // 用最大列数
        let aislePositions: number[] = [];
        if (colCount < 8) {
            aislePositions = [];
        } else if (typeof hallTemplate.aislePositions === 'function') {
            aislePositions = hallTemplate.aislePositions(colCount);
        } else {
            aislePositions = hallTemplate.aislePositions || [];
        }
                    ...
                    {Array.from({ length: colCount }).map((_, index) => {
                        ...
                        return (
                            <React.Fragment key={colNum}>
                                ...
                                {aislePositions.includes(colNum) && <div className={styles['aisle']} />}
                            </React.Fragment>
                        );
                    })}
                </div>
